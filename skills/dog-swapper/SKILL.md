---
name: dog-swapper
description: 将现有壁纸或图片中的狗替换为一张或多张身份参考照片中的同一只真实狗狗，同时保留基础图的场景、构图、姿势、机位和非狗内容。用户要求换狗、替换图片中的狗，或让现有狗狗壁纸中的狗变成自己的狗时使用。
license: MIT
metadata:
  author: RuntianLee
  version: "0.4.1"
...
使用 `prompt.txt` 中的标准指令，将基础图中的狗替换为用户参考照片所指向的那只狗。

## 必需输入

- 恰好一张基础壁纸或基础图，其中包含待替换的狗。
- 一张或多张同一只目标狗的原始照片。
...
- 不得使用生成图、合成壁纸或包含另一只狗的图片作为身份参考。
- 本流程仅用于狗换狗。基础图主体或身份参考属于其他物种时，说明冲突并请求澄清；不得擅自切换 Skill 或尝试跨物种替换。
- 目标主体或输入职责不清楚，或参考中包含不同的狗时，停止并询问缺失的决定。仅有某项表情特征看不清时，按上方的不确定性规则处理。用户未要求生成时，只完成准备工作。
- 保护每张输入图片的隐私和使用权。
- 提示词要求严格保留非狗区域，但生成模型不保证这些区域逐像素不变。
