...
3. 同一基础图中存在多只猫、且每只目标猫的参考组和猫位映射明确时，仍进入 `cat-swapper`，由它选择多猫模式。不同目标猫的参考照片不构成物种冲突。
4. 物种、目标主体或图片职责仍不明确，参考中混有不同物种，或多只同物种宠物的身份分组与猫位映射不清楚时，只询问缺失的决定。猫换狗、狗换猫及其他跨物种替换不在这些流程的适用范围内；说明不支持的情况，不开始生成。

## 继续执行选定的 Skill

- 猫：完整阅读[猫咪专用 Skill](../cat-swapper/SKILL.md)，由它根据待替换猫数量选择单猫或多猫提示词。
...
在当前对话中，继续执行同组本地目录里的选定 Skill，保留用户的附件、图片职责、所选服务商、请求上限和已有授权。已经明确的选择或授权，不要求用户重复提供。猫咪专用 Skill 必须按模式读取恰好一份提示词并准备对应的当次描述；狗狗专用 Skill 继续先写当次表情段，再接上完整且字节不变的 `prompt.txt`。不得复用另一张壁纸的表情段，不得拼接猫狗或单猫／多猫提示词，也不得把本段分流说明加入生图指令。不得擅自使用其他版本的已安装专用 Skill。
