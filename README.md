...
两份提示词均强化了基础图的可见表情继承：对照眼睑位置、眼裂开合、视线、耳位和嘴巴状态，同时保留目标宠物稳定的眼形、眼色与面部结构。基础图睁眼、闭眼或半眯时分别遵循原状态，不统一套用半眯或可爱表情；遮挡和不清楚的部位不臆测。

“表情不匹配即失败”是提示词内的验收要求，不是程序级拦截，也不会触发自动重试。生成后仍需检查实际结果。
...
猫版在单只猫、同一基础壁纸的有限测试中获得“相对初版表情控制有进步”的用户反馈，但严格表情匹配检查仍未通过。狗版尚未经过真实图片模型与主人身份判断；这些结果不证明跨宠物、跨表情稳定或效果已验收。生成模型也不保证非目标区域像素级保持。
