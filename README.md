...
| [`cat-swapper`](skills/cat-swapper/SKILL.md) | 手动选择猫咪专用工作流；支持单猫、一次性多猫直出和分层多猫合成 | 可单独安装 |
...
根目录是仓库说明，已不再作为可安装的 Skill。公共入口没有生图提示词，只在当前对话中读取一个专用 Skill；猫单猫、猫多猫直出和狗单狗提示词各自只有一个文件真源。
...
猫 Skill 遇到两只或更多目标猫时，先在两条路线中选择：

- **分层多猫**：猫位可独立裁出，身份一致和非猫区域保持优先。先生成无猫底图，再逐猫生成局部小图、本地抠成透明层并按坐标合成；逐猫继续复用单猫 `prompt.txt`。
- **一次性多猫直出**：猫之间有复杂接触、遮挡或共享阴影，用户明确要求单次结果，或没有可靠的本地抠像与 alpha 合成能力。使用 `prompt-multi.txt`。

分层流程可以减少同一请求中的身份组混合，并能检查全部猫层 alpha 并集之外的背景像素；代价是请求数随猫数增加，也不适合必须整体生成的复杂猫际关系。它不绑定图片模型或抠像器，每次外部调用仍需独立授权、请求／成本上限和零重试规则。

一个七猫、飞行姿势、猫位可分离的案例获得主人整体接受，并通过本地合成完整性检查；其中严格姿势／表情 QC 没有整体通过，自动抠像也不构成无损毛发证明。该案例不足以删除一次性多猫提示词或证明跨壁纸稳定。

## 动态表情与身份边界（猫 v0.6.0、狗 v0.4.1、公共入口 v0.3.0）

猫咪 Skill 会先根据图 1 中待替换猫的数量和猫际关系选择模式。恰好一只时只读取原有单猫流程和 `prompt.txt`；两只或更多时，分层模式读取 `layered-multi-cat.md` 并逐猫复用 `single-cat.md` 和 `prompt.txt`，一次性直出模式读取 `multi-cat.md` 和 `prompt-multi.txt`。不同模式不拼接执行，单猫提示词保持字节不变。
...
一次性多猫直出模式改用“本次基础图的多猫映射与表情要求”。每个猫位区块必须包含稳定定位、目标猫参考图片编号和独立表情描述；参考组之间不得混合、交换或补齐特征。分层模式则为每只猫单独建立当次表情要求和身份组。两种模式生成后都要逐猫检查身份、表情和动作，再检查猫的总数、猫际关系和非猫内容。

“表情不匹配即失败”是验收要求，不是模型服务的程序级拦截，也不会触发自动重试。两种多猫模式都不证明模型能够稳定接受任意数量的图片或保持多个身份，生成后仍需检查实际结果。
...
无需安装 Skill 也可上传基础壁纸及原始身份参考照片，再把对应提示词完整粘贴给支持多图输入的图像工具。单猫／单狗模式只使用对应 `prompt.txt`；分层多猫模式逐猫使用猫咪 `prompt.txt`；一次性多猫直出必须先写清猫位、参考图片分组及一对一映射，再接上完整 `prompt-multi.txt`。明确要求只生成一张、失败后不自动重试。

只粘贴任一提示词文件都不包含上述读图与当次描述步骤。需要动态表情流程时，先按对应 `SKILL.md` 和模式引用文件准备当次描述，再接上完整通用提示词。

| 提示词 | SHA-256 |
| --- | --- |
| [猫咪单猫／分层逐猫](skills/cat-swapper/prompt.txt) | `c4a5bc29660791242df2c49fbda6576208baaaea00e94fca12fd4efc008dbe96` |
| [猫咪一次性多猫直出](skills/cat-swapper/prompt-multi.txt) | `23bb0b2a20d751a8eb83a414247cf2be1b6dc92aa2fc7ef625c902b33751c554` |
...
猫咪单猫动态描述在同一基础壁纸、两只目标猫分别进行的有限测试中获得正向反馈，也出现过表情接近但脸型失真的情况；加入脸型边界后的单次结果获得用户接受。一次性多猫直出提示词已有一次七猫低清实验，但没有形成获主人接受的终态；分层多猫模式有一个猫位可分离的七猫案例获主人整体接受并通过本地合成完整性检查，但严格姿势／表情 QC 未整体通过，自动抠像也不构成无损毛发证明。两种模式均未证明跨壁纸稳定。狗版同样尚未经过真实图片模型与主人身份判断；文本规则一致不代表生成效果一致。生成模型也不保证非目标区域像素级保持。
