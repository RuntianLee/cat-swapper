...
“本次基础图的表情要求”描述当次可见表情及其与目标宠物身份结构的边界。睁眼、闭眼、半眯或张嘴都按当前图片决定，不统一套用半眯、可爱或不耐烦的描述；允许表情造成的自然局部变化，不改变稳定的头脸结构。公共入口要求读取同组专用 Skill，避免混用旧版本。

本次更新不改变两份通用 `prompt.txt`。“表情不匹配即失败”是验收要求，不是模型服务的程序级拦截，也不会触发自动重试。生成后仍需检查实际结果。
...
只粘贴 `prompt.txt` 不包含上述读图与当次描述步骤。需要动态表情流程时，先按对应 `SKILL.md` 准备当次描述，再接上完整通用提示词。

| 提示词 | SHA-256 |
| --- | --- |
| [猫咪](skills/cat-swapper/prompt.txt) | `c4a5bc29660791242df2c49fbda6576208baaaea00e94fca12fd4efc008dbe96` |
...
猫版动态描述在同一基础壁纸、两只目标猫的有限测试中获得正向反馈，也出现过表情接近但脸型失真的情况；加入脸型边界后的单次结果获得用户接受。这不等于全部技术检查通过，也不证明改进的唯一原因或重复运行稳定。狗版尚未经过真实图片模型与主人身份判断；猫狗流程文本一致不代表生成效果一致，完整留出／回归验证仍未完成。生成模型也不保证非目标区域像素级保持。
